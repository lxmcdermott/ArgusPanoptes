...
def main() -> int:
    ...
    results: dict[str, Any] = {
        "config": {
            ...
            "batch": args.batch, "iters": args.iters, "device": "cpu",
        },
        "models": {},
    }
    ...
    print("=" * 78)
    print(f"{'model':14s} {'p50 (ms)':>10s} {'p95 (ms)':>10s} {'batch p50':>12s} "
          f"{'throughput':>14s}")
    print("-" * 78)
    for name, kind in kinds.items():
        ...
        if r is None:
            print(f"{name:14s}  (missing {onnx_path.name} - skipped)")
            continue
        results["models"][name] = r
        print(f"{name:14s} {r['single']['p50_ms']:10.3f} {r['single']['p95_ms']:10.3f} "
    ...
    if xgb:
        results["xgboost"] = xgb
        print(f"{'xgboost':14s} predict p50={xgb['xgb_predict']['p50_ms']:.3f} ms  "
              f"+ DSP extract p50={xgb['dsp_extract']['p50_ms']:.3f} ms  "
              f"-> end-to-end {xgb['end_to_end_p50_ms']:.3f} ms")
    else:
        print("xgboost        (no xgb_reg_wear_level.joblib - run models/baseline.py first)")
    print("=" * 78)
